...
export function HeroSection() {
  ...
  const [phase, setPhase] = useState<'idle' | 'typing' | 'contextLoaded' | 'splitting' | 'materialized'>('idle')

  useEffect(() => {
    let currentIndex = 0
    let skipTicks = 0
    
    // تأخیر کوتاه قبل از شروع تایپ
  ...
  return (
    <section className="bg-zinc-950 min-h-screen relative flex flex-col items-center p-4 md:p-8 pt-24 md:pt-32 pb-24 overflow-hidden" dir="rtl">
      
      {/* Background ambient glow for glassmorphism visibility */}
      ...
      <div className="flex flex-col items-center text-center gap-6 mb-16 relative z-20">
        ...
        <motion.h1 
          ...
          className="text-4xl md:text-5xl lg:text-7xl font-bold text-zinc-100 max-w-4xl leading-tight"
        >
          کدنویسی با هوش مصنوعی،<br />
          <span className="text-transparent bg-clip-text bg-gradient-to-l from-purple-400 to-blue-400">بدون فراموشی کانتکست</span>
        </motion.h1>
        ...
        >
          کدیار اولین محیط توسعه هماهنگ با حافظه طولانی مدت است. ایده‌ات رو بهش بگو، کد تحویل بگیر و نگران پریدن تمرکز هوش مصنوعی تو پروژه‌های بزرگ نباش.
        </motion.p>
      ...
      {/* IDE Mockup */}
      <div className="w-full max-w-5xl h-[600px] lg:h-[700px] flex flex-col backdrop-blur-xl bg-zinc-900/60 border border-zinc-800 rounded-xl overflow-hidden shadow-2xl relative z-10">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* Sidebar */}
          <div className="w-64 border-l border-zinc-800 bg-zinc-950/40 p-4 flex flex-col gap-1 overflow-y-auto hidden md:flex">
            ...
            <div className="flex items-center gap-2 px-2 py-1.5 text-sm text-purple-200 bg-purple-500/10 border border-purple-500/20 rounded-md transition-colors cursor-default mt-1 relative">
              ...
              {['contextLoaded', 'splitting', 'materialized'].includes(phase) && (
                <div className="flex items-center gap-2 absolute left-2" dir="ltr">
                  <motion.div 
                    initial={{ opacity: 0, scale: 0 }} 
                    animate={{ opacity: 1, scale: 1, boxShadow: ['0 0 4px #4ade80', '0 0 12px #4ade80', '0 0 4px #4ade80'] }}
                    transition={{ 
                ...
                <div className="flex-1 border-l border-zinc-800 bg-zinc-900/10 relative flex items-center justify-center p-6 overflow-hidden">
                  {phase === 'materialized' && (
                    <motion.div 
                      ...
                      className="w-full max-w-sm backdrop-blur-xl bg-zinc-950/80 border border-zinc-800 rounded-xl p-6 shadow-[0_0_40px_rgba(139,92,246,0.15)] flex flex-col gap-4"
                    >
                      <h3 className="text-zinc-100 font-medium">تسک منیجر</h3>
                      <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-3 p-3 rounded-lg border border-zinc-800/50 bg-zinc-900/50">
                          <div className="w-5 h-5 rounded border border-violet-400 ring-4 ring-violet-400/30 flex items-center justify-center">
                            <div className="w-2 h-2 bg-violet-400 rounded-sm"></div>
                          </div>
                          <span className="text-sm text-zinc-300 line-through opacity-70">طراحی UI هیرو سکشن</span>
                        </div>
                        <div className="flex items-center gap-3 p-3 rounded-lg border border-violet-500/30 bg-violet-500/10 shadow-[inset_0_0_20px_rgba(139,92,246,0.1)]">
                          <div className="w-5 h-5 rounded border border-violet-400/70"></div>
                          <span className="text-sm text-zinc-200">پیاده‌سازی انیمیشن‌های Framer</span>
                        </div>
                        <div className="flex items-center gap-3 p-3 rounded-lg border border-zinc-800/50 bg-zinc-900/30">
                          ...
                          <span className="text-sm text-zinc-400">یکپارچه‌سازی دیتابیس</span>
                        </div>
                      </div>
                    </motion.div>
                  )}
